    ...
    def controller_issues_new_after_save(context={ })
      ...
      relevant_users.uniq.each do |user|
        if user.pref[:proxy_user_id] != 0
          ...
          #find stand-ins of stand-ins
          done = 0
          currentID = user.pref[:proxy_user_id]
          until done == 1
            if User.find(currentID).pref[:proxy_user_id] != 0
              if notify_these_ids.include? User.find(currentID).pref[:proxy_user_id]
                done = 1
              else
                notify_these_ids << User.find(currentID).pref[:proxy_user_id]
                currentID = User.find(currentID).pref[:proxy_user_id]
              end
            else
              done = 1 
            end
      ...
      notify_these_ids.uniq.each do |id|
        tempmail = Mailer.issue_add( context[:issue], [User.find_by_id(id)] , [] )
